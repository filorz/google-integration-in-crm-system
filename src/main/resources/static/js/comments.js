    ...
    $.ajax({
        ...
        success: function (comment) {
            ...
            $('#client-' + id + 'comments').prepend(
                ...
                '<span class="comment-name text-dark m-b-10">' + comment.user.lastName + ' ' + comment.user.firstName + '</span>' +
                '<span class="fas fa-window-close comment-functional" onclick="deleteComment(' + comment.id + ')"></span>' +
                '<span class="edit-comment fas fa-pencil-alt comment-functional"></span>' +
                '<span class="hide-show fas fa-comment-dots comment-functional"></span>' +
                '<span  class="comment-functional">'+ dateFormat +'</span>' +
                '<p class="comment-text m-t-10">' + comment.content + '</p>' +
...
$(function () {
    $('#main-modal-window').on('shown.bs.modal', function (event) {
        var client_id = $(this).data('cardId');
        let user_id;
            ...
            for (let i = list.length-1; i >=0; i--) {
                ...
                // if (list[i].mainComment == null) {
                    if (user_id === list[i].user.id + '') {
                        removeComment = '<span class="fas fa-window-close comment-functional d-inline-block m-l-10" onclick="deleteComment(' + list[i].id + ')"></span>'
                        editComment = '<span class="edit-comment fas fa-pencil-alt comment-functional d-inline-block m-l-10"></span>'
                    ...
                    html +=
                        ...
                        '<span class="comment-name text-dark font-bold m-b-10 d-inline-block p-r-10">' + list[i].user.lastName + ' ' + list[i].user.firstName + '</span>' +
                        '<span  class="comment-functional font-12 p-t-2">'+ dateFormat +'</span>' +
                        '<div class="ml-auto">' +
                        '<span class="hide-show fas fa-comment-dots comment-functional d-inline-block m-l-10"></span>' +
                        editComment +
                        removeComment +
                        '</div>' +
                        '</div>' +

                        '<div class="comment-text m-l-20 d-inline-block" ">' + list[i].content + '</div>' +
                        '<div class="form-answer">' +
                        '<div class="form-group">' +
                        '<textarea class="form-control textcomplete" id="new-answer-for-comment' + list[i].id + '" placeholder="Напишите ответ"></textarea>' +
                        '<button class="btn btn-sm btn-success comment-button m-t-10" onclick="sendAnswer(' + list[i].id + ', \'test_message\')"> Сохранить </button>' +
                        '</div>' +
                        '</div>' +
                        '<div class="form-edit">' +
                        '<div class="form-group">' +
                        ...
                        '<button class="btn btn-sm btn-success comment-button m-t-10" onclick="editComment(' + list[i].id + ')"> Отредактировать </button>' +
                        '</div>' +
                        '</div>' +
                    ...
                    for (let i = 0; i < answers.length; i++) {
                        ...
                        if (user_id === answers[i].user.id + '') {
                            removeAnswer = '<span class="fas fa-window-close comment-functional" onclick="deleteCommentAnswer(' + answers[i].id + ')"></span>'
                            editAnswer = '<span class="edit-answer fas fa-pencil-alt comment-functional"></span>'
                        } else {
                            removeAnswer = '';
                            editAnswer = '';
                        }
    ...
    $.ajax({
        ...
        success: function (answer) {
            ...
            $('#comment-' + id + 'answers').prepend(
                ...
                "   <span class='fas fa-window-close comment-functional' onclick='deleteCommentAnswer(" + answer.id + ")'></span>" +
                //edit-comment
                "   <span class='edit-answer fas fa-pencil-alt comment-functional'></span>" +
                '   <span  class="comment-functional">'+ dateFormat +'</span>' +
                //comment-text
