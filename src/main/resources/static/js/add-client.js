$(function () {
    $('.createNewClient').on('click', function () {
        var statusName = $(this).data('statusName');
        var currentModal = $('#add-new-client-modal');
        currentModal.data('statusName', statusName);
        currentModal.modal('show');
    });
});

$(function () {
    $('#add-new-client-modal').on('show.bs.modal', function () {
        var statusName = $(this).data('statusName');
        $('#client-status').text(statusName);
        $.ajax({
            type: 'GET',
            url: '/rest/status/all-statuses',
            success: function (entity) {
                $.each(entity, function(i, p) {
                    $('#edit-client-state').append($('<option></option>').val(p).html(p));
                });
            },
...
function addNewSN() {
	...
	$("#SN-table-body").append("<tr><td hidden=\"hidden\"></td><td></td><td></td><td><button type=\"button\" onclick=\"deleteSocial(this)\" class=\"fas fa-window-close\"></button></td></tr>")
}

function addNewJob() {
	var size =  ($("#job-table-body")[0]).rows.length;
	$("#job-table-body").append("<tr><td hidden=\"hidden\"></td><td></td><td></td><td><button type=\"button\" onclick=\"deleteJob(this)\" class=\"fas fa-window-close\"></button></td></tr>")
}
