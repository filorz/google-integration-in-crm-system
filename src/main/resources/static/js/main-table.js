    ...
    $.ajax({
        ...
        success: function (owner) {
            assignBtn.before(
                ...
                "   onclick='unassign(" + id + ")' " +
                "   class='btn btn-outline-dark btn-sm remove-tag position-relative m-t-10'>" +
                " <i class='fas fa-times font-12 d-inline-block'></i> Снять менеджера</a>"
            );
            assignBtn.remove();
            $('#info-client' + id).append(
                "<span class='user-icon d-inline-block rounded-circle user-avatar-md font-12 text-center p-t-7' id='own-" + id + "' value=" + owner.firstName + "&nbsp" + owner.lastName + ">" +
...
function assignUser(id, user, principalId) {
    ...
    $.ajax({
        ...
        success: function (owner) {
            ...
            info_client.append(
                "<span class='user-icon d-inline-block rounded-circle user-avatar-md font-12 text-center p-t-7' id='own-" + id + "' value=" + owner.firstName + " " + owner.lastName + ">" +
                owner.firstName.substring(0, 1) + owner.lastName.substring(0, 1) +
                "</span>"
            );
            fillFilterList()
        ...
        success: function (owner) {
            ...
            if (unassignBtn.length !== 0) {
                unassignBtn.before(
                    ...
                    "   id='assign-client" + id + "' " +
                    "   onclick='assign(" + id + ")' " +
                    "   class='card-link remove-tag text-primary font-bold m-15 d-inline-block'><i class=\"fas fa-hand-point-right d-inline-block m-r-10\"></i> Взять себе карточку</a>"
                );
                unassignBtn.remove();
            }
            fillFilterList();
...
function assignSkype(id) {
    ...
    var btnBlockTask = $('div.confirm-skype-interceptor .assign-skype-call-btn');
    var currentBtn = $(document).find('.assign-skype-call-btn');
    currentBtn.addClass("not-active").removeClass("text-primary");
    var actionBox = $('.skype-call-body');
    var currentStatus = $('.skype-notification');
    var formData = {clientId: clientId};
    var nowDate = new Date();
    var minutes =  Math.ceil((nowDate.getMinutes() +1)/10)*10;
...
$(function () {
    $('.card-main').on('click', function () {
        var clientId = $(this).data('cardId');
        var currentModal = $('#main-modal-window');
        currentModal.data('cardId', clientId);
            ...
            success: function(client) {
                $.get('rest/client/getPrincipal', function (user) {
                    ...
                    if (client.ownerCallSkype === null) {
                        btnBlock.after('<div class="remove-tag confirm-skype-interceptor"><a href="#" id="assign-skype' + client.id + '" onclick="assignSkype(' + client.id + ')" class="font-bold text-primary assign-skype-call-btn m-15 d-inline-block"><i class="fas fa-phone font-12 d-inline-block m-r-10"></i> Назначить беседу в Skype</a>\n' +
                            '<div class="skype-notification"></div>\n' +
                            '</div>')
                    }
                    if (client.ownerCallSkype !== null) {
                    ...
                    if (client.ownerUser === null) {
                        btnBlock.append('<a href="#" class="remove-tag text-primary font-bold m-15 d-inline-block" id="assign-client' + client.id + '"onclick="assign(' + client.id + ')"><i class="fas fa-hand-point-right d-inline-block m-r-10"></i> Взять себе карточку </a>');
                    }
                    if (client.ownerUser !== null) {
                        btnBlock.prepend('<a href="#" class="btn position-relative m-t-10 btn-outline-dark btn-sm remove-tag" id="unassign-client' + client.id + '" onclick="unassign(' + client.id + ')"><i class=\'fas fa-times font-12 d-inline-block\'></i> Снять менеджера</a>');
                    }
                });
                ...
                $('.comment-div').append('<button class="btn btn-primary btn-sm comment-button remove-element m-t-20" id="assign-client' + client.id + '"  onclick="sendComment(' + client.id + ', \'test_message\')"> Добавить комментарий </button>');
